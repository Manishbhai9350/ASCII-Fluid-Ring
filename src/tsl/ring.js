import { Mesh, MeshBasicNodeMaterial, PlaneGeometry, RenderTarget, Scene, Uniform } from "three/webgpu";
import {
  float,
  If,
  PI,
...
export const CreateRingTexture = (sw,sh,config) => {
    const ringScene = new Scene()
    const renderTarget = new RenderTarget(innerWidth,innerHeight)

    

    const RingPlane = new Mesh(
        new PlaneGeometry(sw,sh),
        GetRingMaterial(config)
    )

    ringScene.add(RingPlane)

    return {
        ringScene,
        renderTarget,
        ringTexture:renderTarget.texture
    }
}